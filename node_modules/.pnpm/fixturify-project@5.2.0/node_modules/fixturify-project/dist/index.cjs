...
var Project = class {
  constructor(first, second, third, fourth) {
    ...
    if (files) {
      this.files = __spreadValues(__spreadValues({}, defaultFiles), files);
    } else {
  ...
  resolveLinks() {
    return new Map([...this.dependencyLinks.entries()].map(([name, opts]) => {
      let dir;
      if ("baseDir" in opts) {
    ...
    for (let section of ["dependencies", "peerDependencies"]) {
      if (targetPkg[section]) {
        for (let depName of Object.keys(targetPkg[section])) {
          ...
          if (!depTarget) {
            throw new Error(`[FixturifyProject] package ${name} in ${target} depends on ${depName} but we could not resolve it`);
          }
...
Project.prototype.writeSync = (0, import_util.deprecate)(Project.prototype.writeSync, "project.writeSync() is deprecated. Use await project.write() instead");

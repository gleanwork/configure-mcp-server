...
import { getServerUrlHash, writeJsonFile as writeMcpRemoteJSONFile, } from '@gleanwork/connect-mcp-server/lib';
import { getConfig, isBasicConfig, isGleanTokenConfig, isOAuthConfig, } from '../config/index.js';
import { debug, error, trace } from '../log/logger.js';
import { loadTokens, saveTokens, Tokens } from './token-store.js';
import { isAuthResponse, isAuthResponseWithURL, isTokenSuccess, } from './types.js';
 ...
 */
export async function setupMcpRemote(opts) {
    const config = await getConfig();
    if (isGleanTokenConfig(config)) {
        throw new AuthError('Cannot setup MCP remote with Glean token configuration. Please use OAuth configuration instead.', { code: AuthErrorCode.GleanTokenConfigUsedForOAuth });
    }
    const origin = new URL(config.baseUrl).origin;
    const serverUrl = opts.target === 'agents'
    ...
    if (clientData === null) {
        throw new AuthError('Missing OAuth metadata required for MCP remote setup. Please authenticate first using OAuth.', { code: AuthErrorCode.MissingOAuthMetadata });
    }
    const tokens = loadTokens();
    if (tokens == null) {
        throw new AuthError('Missing OAuth tokens required for MCP remote setup. Please authenticate first using OAuth.', { code: AuthErrorCode.MissingOAuthTokens });
    }
    const mcpRemoteClientInfo = {
    ...
    if (clientData === null || clientData === void 0 ? void 0 : clientData.clientSecret) {
        mcpRemoteClientInfo.client_secret = clientData === null || clientData === void 0 ? void 0 : clientData.clientSecret;
    }
    await writeMcpRemoteJSONFile(serverHash, 'client_info.json', mcpRemoteClientInfo);
    const mcpRemoteTokens = {
 ...
 * the user code).
 */
export async function forceAuthorize(config) {
    if (config === undefined || isBasicConfig(config)) {
        config = await getConfig({ discoverOAuth: true });
    }
    if (isGleanTokenConfig(config)) {
        throw new AuthError(`Cannot get OAuth access token when using glean-token configuration.  Specify GLEAN_OAUTH_ISSUER and GLEAN_OAUTH_CLIENT_ID and not GLEAN_API_TOKEN to use OAuth.`, { code: AuthErrorCode.GleanTokenConfigUsedForOAuth });
    }
...
export async function attemptUpgradeConfigToOAuth(config) {
    if (isGleanTokenConfig(config)) {
        return config;
    }
...
export async function discoverOAuthConfig(config) {
    ...
    if (isGleanTokenConfig(config)) {
        throw new AuthError('[internal error] attempting OAuth flow with a Glean-issued non-OAuth token', { code: AuthErrorCode.InvalidConfig });
    }
    ...
    if (typeof issuer !== 'string' || typeof clientId !== 'string') {
        ...
        clientSecret = resourceMetadata === null || resourceMetadata === void 0 ? void 0 : resourceMetadata.clientSecret;
    }
    else {
        trace('using environment variables for issuer and client id');
    }
    const { deviceAuthorizationEndpoint, tokenEndpoint } = await fetchAuthorizationServerMetadata(issuer);
    const oauthConfig = {
...
function failAuthorizationServerMetadataFetch(cause) {
    throw new AuthError('Unable to fetch OAuth authorization server metadata: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.AuthServerMetadataNetwork, cause });
}
async function fetchOpenIdConfiguration(issuer) {
...
async function fetchOauthAuthorizationServerConfig(issuer) {
...
export async function fetchAuthorizationServerMetadata(issuer) {
    ...
    try {
        ...
        trace('Falling back to', `${issuer}/.well-known/oauth-authorization-server`, cause);
    ...
    try {
        ...
        throw new AuthError('Unable to fetch OAuth authorization server metadata: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.AuthServerMetadataParse, cause });
    }
    const deviceAuthorizationEndpoint = responseJson['device_authorization_endpoint'];
    const tokenEndpoint = responseJson['token_endpoint'];
    if (typeof tokenEndpoint !== 'string') {
        throw new AuthError('OAuth authorization server metadata did not include a token endpoint: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.AuthServerMetadataMissingTokenEndpoint });
    }
    if (typeof deviceAuthorizationEndpoint !== 'string') {
        throw new AuthError('OAuth authorization server metadata did not include a device authorization endpoint: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.AuthServerMetadataMissingDeviceEndpoint });
    }
    return {
...
export async function fetchProtectedResourceMetadata(config) {
    const origin = new URL(config.baseUrl).origin;
    ...
    try {
        ...
        error(cause);
        throw new AuthError('Unable to fetch OAuth protected resource metadata: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.ProtectedResourceMetadataNetwork, cause });
    }
    if (!response.ok) {
        throw new AuthError('Unable to fetch OAuth protected resource metadata: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.ProtectedResourceMetadataNotOk });
    }
    let responseJson;
    try {
        responseJson = (await response.json());
        ...
        throw new AuthError('Unexpected OAuth protected resource metadata: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.ProtectedResourceMetadataParse, cause });
    }
    ...
    if (typeof issuer !== 'string') {
        throw new AuthError('OAuth protected resource metadata did not include any authorization servers: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.ProtectedResourceMetadataMissingAuthServers });
    }
    if (typeof clientId !== 'string') {
        throw new AuthError('OAuth protected resource metadata did not include a device flow client id: please contact your Glean administrator and ensure device flow authorization is configured correctly.', { code: AuthErrorCode.ProtectedResourceMetadataMissingClientId });
    }
    const result = {
        issuer,
        clientId,
...
export async function forceRefreshTokens() {
    ...
    if (isGleanTokenConfig(config)) {
        throw new AuthError(`Cannot refresh OAuth access token when using glean-token configuration.  Specify GLEAN_OAUTH_ISSUER and GLEAN_OAUTH_CLIENT_ID and not GLEAN_API_TOKEN to use OAuth.`, { code: AuthErrorCode.GleanTokenConfigUsedForOAuthRefresh });
    }
    let tokens = loadTokens();
    if (tokens === null) {
 ...
 */
async function fetchTokenViaRefresh(tokens, config) {
    var _a;
    ...
    trace((_a = options.method) !== null && _a !== void 0 ? _a : 'GET', url, options);
    ...
    if (isTokenSuccess(response)) {
        ...
        trace('/token', errorResponse === null || errorResponse === void 0 ? void 0 : errorResponse.error);
        throw new AuthError(`Unable to fetch token.  Server responded ${responseRaw.status}: ${errorResponse === null || errorResponse === void 0 ? void 0 : errorResponse.error}`, { code: AuthErrorCode.FetchTokenServerError, cause: errorResponse });
    }
}
async function authorize(config) {
    var _a;
    trace('Starting OAuth authorization flow');
    const domain = (_a = parseDomain(config.issuer).domain) !== null && _a !== void 0 ? _a : '';
    ...
    if (!process.stdin.isTTY) {
        throw new AuthError('OAuth device authorization flow requires an interactive terminal.', { code: AuthErrorCode.NoInteractiveTerminal });
    }
    ...
    try {
        ...
        promptUserAndOpenVerificationPage(authResponse, abortController.signal).catch((e) => {
        ...
        const tokenResponse = polledTokenResponse;
        if (tokenResponse.refresh_token === undefined) {
            throw new AuthError(`Your OAuth Authorization Server issued an access token but not a refresh token.  Please configure your OAuth application with id: ${config.clientId} to issue refresh tokens.`, { code: AuthErrorCode.RefreshTokenNotIssued });
        }
 ...
 */
export function getOAuthScopes(config) {
    var _a;
    const { issuer: issuer } = config;
    const domain = (_a = parseDomain(issuer).domain) !== null && _a !== void 0 ? _a : '';
...
export async function fetchDeviceAuthorization(config) {
    var _a;
    const params = new URLSearchParams();
    params.set('client_id', config.clientId);
    ...
    trace((_a = options.method) !== null && _a !== void 0 ? _a : 'GET', url, options.headers, Object.fromEntries(params.entries()));
    ...
    if (!(response.ok &&
        ...
        throw new AuthError('Error obtaining auth grant', {
            ...
            cause: new Error(JSON.stringify({ status: response.status, body: responseJson })),
        });
    }
    const result = Object.assign({}, responseJson);
...
async function pollForToken(authResponse, config) {
    return new Promise((resolve, reject) => {
        ...
        const poll = async () => {
            ...
            if (now - startTime >= timeoutMs) {
                reject(new AuthError('OAuth device flow timed out after 10 minutes. Please try again.', { code: AuthErrorCode.OAuthPollingTimeout }));
                return;
            }
            ...
            if (isTokenSuccess(response)) {
                ...
                if (errorResponse.error == 'authorization_pending') {
                    setTimeout(poll, authResponse.interval * 1000);
                }
                else {
                    reject(new AuthError('Unexpected error requesting authorization grant', {
                        code: AuthErrorCode.UnexpectedAuthGrantError,
                        cause: errorResponse,
                    }));
                }
...
async function waitForUserEnter(signal) {
...
async function promptUserAndOpenVerificationPage(authResponse, signal) {
    console.log(`

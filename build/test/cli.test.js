...
function normalizeBaseDirOutput(output, baseDir) {
...
function normalizeVSCodeConfigPath(output) {
    // Normalize VS Code config paths (both mcp.json and settings.json)
    return output
        .replace(/[^\s"]*(\.config|Library\/Application Support|Code)([/\\][^/\\]+)*[/\\]mcp\.json/g, '<VS_CODE_CONFIG_DIR>/mcp.json')
        .replace(/[^\s"]*(\.config|Code|Application Support)([/\\][^/\\]+)*[/\\]settings\.json/g, '<VS_CODE_CONFIG_DIR>/settings.json');
}
function normalizeClaudeConfigPath(output) {
    ...
    return output.replace(/[^\s"]*(Library\/Application Support\/Claude|Claude)[/\\]claude_desktop_config\.json/g, '<CLAUDE_CONFIG_DIR>/claude_desktop_config.json');
}
function createConfigFile(configFilePath, config) {
...
describe('CLI', () => {
    ...
    it('handles invalid clients', async () => {
        const result = await runBin('--client', 'invalid-client', '--instance', 'my-company');
        expect(result.exitCode).toEqual(1);
        expect(result.stderr).toMatchInlineSnapshot(`
    ...
    for (const command of ['local', 'remote']) {
        ...
        `);
            ...
            it('fails when only instance provided', async () => {
                const result = await runBin('--client', 'cursor', '--instance', 'test-instance', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
                ...
                expect(result.exitCode).toEqual(1);
                expect(result.stderr).toMatchInlineSnapshot(`"Error configuring client: Local configuration requires an API token (--token). Please provide it via command line options or in your .env file."`);
            });
            it('fails when env file has only token', async () => {
                ...
                expect(result.exitCode).toEqual(1);
                expect(result.stderr).toMatchInlineSnapshot(`"Error configuring client: Local configuration requires an instance (--instance) or URL. Please provide it via command line options or in your .env file."`);
            });
    ...
    describe('local', () => {
        it('can configure with custom instance and token', async () => {
            const result = await runBin('--client', 'cursor', '--instance', 'custom-instance', '--token', 'test-token', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
        ...
        describe('Claude Code client', () => {
            ...
            it('creates a new config file when none exists', async () => {
                const result = await runBin('--client', 'claude-code', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
        ...
        it('uses token auth when both token and instance provided via flags', async () => {
            const result = await runBin('--client', 'cursor', '--token', 'test-token', '--instance', 'test-instance', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
      ...
      `);
        ...
        it('prioritizes flags over env file when both provided', async () => {
            ...
            const result = await runBin('--client', 'cursor', '--token', 'flag-token', '--instance', 'flag-instance', '--env', envFilePath, {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
      ...
      `);
        ...
        it('warns when env file path does not exist', async () => {
            ...
            const result = await runBin('--client', 'cursor', '--env', nonExistentPath, {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
      ...
      `);
        ...
        describe('Cursor client', () => {
            ...
            it('creates a new config file when none exists', async () => {
                const result = await runBin('--client', 'cursor', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
        ...
        describe('Claude client', () => {
            ...
            it('creates a new config file when none exists', async () => {
                const result = await runBin('--client', 'claude', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
        ...
        describe('Claude Code client', () => {
            ...
            it('creates a new config file when none exists', async () => {
                const result = await runBin('local', '--client', 'claude-code', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                    },
                });
        ...
        describe('Windsurf client', () => {
            ...
            it('creates a new config file when none exists', async () => {
                const result = await runBin('--client', 'windsurf', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
        ...
        describe('Goose client', () => {
            ...
            it('creates a new config file when none exists', async () => {
                const result = await runBin('--client', 'goose', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
        ...
        describe('VS Code client', () => {
            ...
            beforeEach(() => {
                ...
                if (platform === 'win32') {
                    configFilePath = path.join(project.baseDir, 'Code', 'User', 'mcp.json');
                }
                else if (platform === 'darwin') {
                    configFilePath = path.join(project.baseDir, 'Library', 'Application Support', 'Code', 'User', 'mcp.json');
                }
                else {
                    configFilePath = path.join(project.baseDir, '.config', 'Code', 'User', 'mcp.json');
                }
                fs.mkdirSync(path.dirname(configFilePath), { recursive: true });
            });
            it('creates a new config file when none exists', async () => {
                const result = await runBin('--client', 'vscode', '--token', 'glean_api_test', '--instance', 'test-domain', {
                    env: {
                        GLEAN_MCP_CONFIG_DIR: project.baseDir,
                        HOME: project.baseDir,
                        USERPROFILE: project.baseDir,
    ...
    describe('remote', () => {
        ...
        it('requires --url for remote configuration', async () => {
            const result = await runBin('remote', '--client', 'cursor', '--token', 'glean_api_test', '--instance', 'test-domain', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
            expect(result.exitCode).toEqual(1);
            expect(result.stderr).toContain('Remote configurations require a full URL (--url)');
        });
        it('configures with --url for full MCP server URLs', async () => {
            const result = await runBin('remote', '--url', 'https://my-be.glean.com/mcp/analytics', '--client', 'cursor', '--token', 'test-token', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
          ...
          "
        `);
            const configFileContents = fs.readFileSync(path.join(project.baseDir, '.cursor', 'mcp.json'), 'utf8');
      ...
      `);
        ...
        it('uses "glean" as server name when URL contains /mcp/default', async () => {
            const result = await runBin('remote', '--url', 'https://my-be.glean.com/mcp/default', '--client', 'cursor', '--token', 'test-token', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
      ...
      `);
        ...
        it('warns when --agents is used with --url', async () => {
            const result = await runBin('remote', '--url', 'https://my-be.glean.com/mcp/custom-agent', '--agents', '--client', 'cursor', '--token', 'test-token', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
            ...
            expect(normalizeOutput(result.stderr, project.baseDir)).toContain('Note: --agents flag is ignored when using --url');
            const configFileContents = fs.readFileSync(path.join(project.baseDir, '.cursor', 'mcp.json'), 'utf8');
            const config = JSON.parse(configFileContents);
        ...
        it('warns when both --url and --instance are provided and ignores instance', async () => {
            const result = await runBin('remote', '--url', 'https://my-be.glean.com/mcp/analytics', '--instance', 'test-instance', '--client', 'cursor', '--token', 'test-token', {
                env: {
                    GLEAN_MCP_CONFIG_DIR: project.baseDir,
                },
            });
            ...
            const output = normalizeOutput(result.stdout + result.stderr, project.baseDir);
            expect(output).toContain('Warning: Both --instance and --url were provided. The --instance flag will be ignored when --url is specified.');
            ...
            expect(config.mcpServers.glean_analytics.url).toBe('https://my-be.glean.com/mcp/analytics');
        });
    });
});

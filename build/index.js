...
import { configure, listSupportedClients, validateFlags, } from './configure/index.js';
import { availableClients, ensureClientsLoaded, } from './configure/client/index.js';
...
async function main() {
    ...
    try {
        ...
        console.error('Warning: Failed to load client modules. Help text may be incomplete.');
    }
...
`;
    ...
    switch (command) {
        case 'remote': {
            const { client, token, instance, url, env, workspace, agents } = cli.flags;
            ...
            if (url && agents) {
                console.warn('Note: --agents flag is ignored when using --url. The server is determined by the URL path.\n');
            }
            try {

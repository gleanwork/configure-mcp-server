...
import { availableClients, ensureClientsLoaded, } from './client/index.js';
    ...
    if (options.envPath) {
        try {
            ...
            if (!fs.existsSync(envPath)) {
                ...
                if ((_a = envConfig.parsed) === null || _a === void 0 ? void 0 : _a.GLEAN_URL) {
                    result.url = envConfig.parsed.GLEAN_URL;
                }
                else if (((_b = envConfig.parsed) === null || _b === void 0 ? void 0 : _b.GLEAN_INSTANCE) ||
                    ((_c = envConfig.parsed) === null || _c === void 0 ? void 0 : _c.GLEAN_SUBDOMAIN)) {
                    result.instance =
                        envConfig.parsed.GLEAN_INSTANCE || envConfig.parsed.GLEAN_SUBDOMAIN;
                }
                result.apiToken = (_d = envConfig.parsed) === null || _d === void 0 ? void 0 : _d.GLEAN_API_TOKEN;
            }
        }
        catch (error) {
 ...
 */
export async function configure(client, options) {
    ...
    if (!availableClients[normalizedClient]) {
        ...
        console.error('Supported clients: ' + Object.keys(availableClients).join(', '));
        process.exit(1);
    }
    // Validate --workspace flag is only used with VS Code
    if (options.workspace && normalizedClient !== 'vscode') {
        console.error('Configuration failed: --workspace flag is only supported for VS Code');
        process.exit(1);
    }
    // Handle conflicting --instance and --url flags
    if (options.instance && options.url) {
        console.warn('Warning: Both --instance and --url were provided. The --instance flag will be ignored when --url is specified.');
        delete options.instance;
    }
    // For remote configurations, require a URL, not an instance name
    if (options.remote && options.instance && !options.url) {
        console.error('Configuration failed: Remote configurations require a full URL (--url), not an instance name (--instance)');
    ...
    if (options.instance && process.env._SKIP_INSTANCE_PREFLIGHT !== 'true') {
        ...
        if (!(await validateInstance(options.instance))) {
            ...
            console.error();
            console.error(`Unable to establish a connection to Glean instance: ${options.instance}`);
            console.error();
            console.error('Troubleshooting tips:');
            console.error('1. Check that the instance name or URL is correct');
            console.error('  • Visit https://app.glean.com/admin/about-glean and look for "Server instance"');
    ...
    try {
        ...
        if (options.remote) {
            ...
            if (!url) {
                throw new Error('Remote configuration requires a URL (--url). Please provide it via command line options or in your .env file.');
            }
            if (apiToken) {
            ...
            if (!instance && !url) {
                throw new Error('Local configuration requires an instance (--instance) or URL. Please provide it via command line options or in your .env file.');
            }
            if (!apiToken) {
                throw new Error('Local configuration requires an API token (--token). Please provide it via command line options or in your .env file.');
            }
        ...
        const newConfig = clientConfig.configTemplate(instanceOrUrl, apiToken, options);
 ...
 */
function writeConfigFile(configFilePath, newConfig, clientConfig, options) {
    ...
    if (fs.existsSync(configFilePath)) {
        ...
        try {
            ...
            console.error(`Error parsing existing configuration file: ${error.message}`);
            console.error(`Creating backup of existing file to ${backupPath} and creating new one...`);
        ...
        existingConfig = clientConfig.updateConfig(existingConfig, newConfig, options);
        if (isYaml) {
...
export async function listSupportedClients() {
    ...
    if (clients.length === 0) {
        console.log('No clients found. This may be an issue with the configuration.');
    }
    else {
    ...
    console.log('\nUsage:');
    console.log('  npx @gleanwork/configure-mcp-server --client <client> [--token <token>] [--instance <instance>]');
    console.log('  npx @gleanwork/configure-mcp-server --client <client> --env <path-to-env-file>');
    console.log('\nExamples:');
    if (clients.length > 0) {
        const exampleClient = clients[0][0];
        console.log(`  npx @gleanwork/configure-mcp-server --client ${exampleClient} --token your-token --instance your-instance`);
        console.log(`  npx @gleanwork/configure-mcp-server --client ${exampleClient} --env ~/.glean.env`);
    }
    console.log(`\nVersion: v${VERSION}`);
}
/**
 ...
 */
export async function validateFlags(client, token, instance, url, env) {
    if (!client) {
    ...
    const hasEnvironmentInstance = Boolean(process.env.GLEAN_INSTANCE ||
    ...
    if (!hasAnyInstance && !hasAnyToken && !hasEnvParam) {
        ...
        console.error('  1. Both --token and --instance for local configuration, or');
